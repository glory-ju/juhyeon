        ...
        if y is not None:
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    # print()
                    print(' '*8 + '|==>  acc: %.4f,  nmi: %.4f  <==|' % (acc(self.y, y_pred), nmi(self.y, y_pred)))
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                ...
                if y is not None:
                    ...
                    logdict = dict(iter=ite, acc=acc_, nmi=nmi_, ari=ari_, loss=loss)
...
def dec(df, n_clusters, **param):
    # setting the hyper parameters
    import argparse

    parser = argparse.ArgumentParser(description='train',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--dataset', default='doc2vec',
    ...
    if args.ae_weights is None:
        dec.pretrain(x=x, y=y, optimizer=pretrain_optimizer,
                     ...
                     save_dir=args.save_dir, n_clusters=n_clusters)
    else:
